...
function helpGenerator(target) {
    switch (target) {
        case 'chat':
            return `\`${target}\` コマンドは，\`gpt-4-turbo-preview\` を用いて，\`質問\` に対する回答を生成します．
...
\> \`Mermaid\` 図形を生成します．`;
        case 'image':
            return `\`${target}\` コマンドは，\`DALL･E\` を用いて，\`質問\` に対するイラストを生成します．
\`画像サイズ\` は生成速度に影響します．`;
        case 'translate':
            return `\`${target}\` コマンドは，\`gpt-4-turbo-preview\` を用いて，\`原文\` を \`対象言語\` に翻訳します．
